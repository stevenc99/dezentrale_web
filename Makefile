...
define CMDS
ifeq ($(1), runserver)
	envdir envs/$(ENV) dezentrale_web/manage.py$(1)$(PORT)
else
$(1):
	envdir envs/$(ENV) dezentrale_web/manage.py$(1)
endif
endef
...
create-db:
	envdir envs/$(ENV) createdb -U dezentrale_web -l en_US.utf-8 -E utf-8 -O dezentrale_web -T template0 -e dezentrale_web

create-db-user:
	psql -d postgres -c "CREATE USER \"dezentrale_web\" WITH PASSWORD 'dezentrale_web' CREATEDB;"

drop-db:
	envdir envs/$(ENV) dropdb -i -e -U dezentrale_web dezentrale_web

drop-db-user:
	dropuser -i -e dezentrale_web
...
isort:
	isort --recursive setup.py dezentrale_web/ tests/
...
startapp:
	...
	envdir envs/$(ENV) python manage.py startapp $$app_name dezentrale_web/apps/$$app_name --template dezentrale_web/config/app_template; \
	echo "Don't forget to add 'dezentrale_web.apps."$$app_name".apps."$$app_name_title"Config' to INSTALLED_APPS in 'dezentrale_web.config/settings/common.py'!"
